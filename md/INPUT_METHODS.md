...
> **Project-agnostic version:** the reusable patterns here (the Input-vs-Logic
> split, interchangeable Input sources behind one shape, feature-detection over
> UA sniffing, and the gesture-detection gotchas — identifier-scoped touches,
> document/window double-listener, native-recognizer conflicts, frame-clock
> timing) are distilled, free of this project's specifics, in
> `~/Syncthing/🗣️ claude conversations/threejs/INPUT_DEVICES_AND_HANDLING_THREEJS.md`
> (shared across Three.js projects). That doc holds the general pattern; this one
> keeps the project-specific detail and history.
...
not ECS patterns in general. For brainstorming on device classes beyond
mouse+keyboard/touch (gamepad, WebHID) that aren't built yet, see
`INPUT_DEVICE_EXPANSION.md`.
...
touch listeners on *both* `document` and `window` silently corrupts any
state computed by *diffing across events* (a drag delta, a tap's elapsed
duration) - see `GOTCHAS.md`'s "Registering the same DOM listener on both
`document` and `window` double-processes hand-computed deltas" section for
the full story. Both touch Input classes register on `document` only for
this reason.

## Native browser gesture recognizers compete with custom gesture detection

A real bug, caught only by testing on an actual Android device, where the
double-tap-and-hold-to-walk gesture conflicted with mobile browsers' own
native gesture recognizers (double-tap-to-zoom, then separately a
long-press-gesture timer) in ways that took three fix attempts across two
sessions to fully resolve. See `GOTCHAS.md`'s "Native browser touch-gesture
recognizers compete with custom gesture detection" section for the complete
incident, and `TODO.md` item 13 for the day-by-day history.
