...
- **Model registry** — `{ modelName: loaderFn }`, a **hand-listed lazy loader map** (one entry
  per model, e.g. `{ "Icosahedron": modelIcosahedron }` where `modelIcosahedron()` returns
  ...
  **`assets/models/`** (bundled source, not `public/`, per the §6 steer). The `import.meta.glob`
  auto-collect form is the deferred scale-up (`SCALE_WHEN_NEEDED.md`). Full rationale in §8.
...
- **Lazy, not eager.** A **`modelRegistry`** (name → loader fn), **hand-listed** for now (one
  entry per model, each a `() => import('…obj?url')`), so each model is its own on-demand chunk
  fetched **only when a world that uses it is generated**, never on startup. (Same lazy strategy
  §7 step 4 earmarks for worlds; the `import.meta.glob` auto-collect form is the deferred
  scale-up — `SCALE_WHEN_NEEDED.md`.) Futureproofs for many/large worlds — a world never picked
  pays nothing.
- **`?url` + `OBJLoader.loadAsync` (async), not `?raw` + `parse`** — a large model should be
