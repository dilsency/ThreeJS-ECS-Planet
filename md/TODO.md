<!-- PROVENANCE: Ported verbatim from the seed project ThreeJS-PWA-npm-Vite-Surface-Stable-Dithering (2026-09-10), to resolve the in-code "see <doc>.md" references ECS-Planet inherited when its code was copied from that project. It describes the SEED architecture; some sections cover features stripped from ECS-Planet (the 3D HUD, the dithering shader, multiplayer) or seed-only code (e.g. applyDefaultPresetOnceCubeReady). Treat as inherited reference, not a description of current ECS-Planet code. -->
...
**Update - likely real cause identified from overlay data, fix applied,
awaiting on-device confirmation.** Overlay readout: while standing still
and aiming, the camera-look `touchmove` handler's `dx`/`dy` line updates
normally; while walking (double-tap-and-hold), that line doesn't update at
all - not `dx=0`, the handler doesn't appear to run at all, since even its
own early-return branches call `debugOverlaySetLine()` and neither of
those showed up either. `touch-action: none` only suppresses the
browser's pan/zoom gesture recognizer, not Android/Chrome's separate
long-press-to-select-text/callout heuristic, which is gated by
`user-select`/`-webkit-touch-callout` - neither was set anywhere in
`index.html`. Holding the second tap of the double-tap is exactly the
duration that heuristic fires on; once the browser recognizes it, it takes
over the touch stream for its own text-selection UI and stops delivering
`touchmove` to page listeners entirely, which would produce exactly this
symptom (works fine on a quick continuous drag, breaks specifically once a
touch is *held*). Fix: added `-webkit-touch-callout:none;
-webkit-user-select:none; user-select:none;` to `index.html`'s existing
`html,body` rule, alongside `touch-action:none`. Verified via `npm run
build` that the rule reaches `dist/index.html` unmangled. Not yet
confirmed on real Android hardware - that's the only way to know if this
was actually the cause, the same way the original bug and the
`touch-action` fix's shortfall were both only confirmed there.

**Update - the `user-select`/`-webkit-touch-callout` fix above did not
work either; real cause found via a reported workaround.** Real retesting
found: holding the walking finger down and dragging it still doesn't aim
the camera - but if a *second* finger touches down while the first is
still held, the first finger's movement immediately starts aiming the
camera, still while walking. That behavior pinpoints the actual
mechanism: a stationary, held single touch arms the browser's own
long-press gesture-detection timer (separate from both `touch-action` and
`user-select`/`-webkit-touch-callout`, neither of which address the timer
itself, only what it would visually produce once fired); once that timer
fires mid-hold, the browser stops delivering `touchmove` for that touch
to page listeners entirely. A second touch point starting cancels that
pending timer (multi-touch reads as "not a long-press candidate" to the
browser's own heuristic), which is why the first finger's movement starts
working again the instant the second finger lands.

Fix: call `e.preventDefault()` in the `touchstart` handler itself (not
just `touchmove`, where it was already being called) for both
`EntityComponentCameraControllerFirstPersonInputTouch.methodEventOnTouchStart()`
and `EntityComponentPlayerControllerInputTouch.methodEventOnTouchStart()`
- telling the browser up front that the touch is fully handled by the
page, before its long-press timer gets a chance to arm at all, rather
than trying to react after the fact. The player-controller class's
`touchstart` listener also needed `{passive: false}` added to its
registration, since it wasn't previously registered that way and
`preventDefault()` is a no-op (and throws a console warning) on a passive
listener. Verified via `npm run build`. Not yet confirmed on real Android
hardware - same as every fix attempt in this item, only the reporter's
device can confirm whether the timer-arming theory is actually correct.
